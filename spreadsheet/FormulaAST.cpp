...
public:
    ...
    virtual double Evaluate(const ValGetter& val_getter) const = 0;
...
public:
    ...
    double Evaluate(const ValGetter& val_getter) const override {
        double res;
        switch (type_) {
            case Add:
                res = lhs_->Evaluate(val_getter) + rhs_->Evaluate(val_getter);
                break;
            case Subtract:
                res = lhs_->Evaluate(val_getter) - rhs_->Evaluate(val_getter);
                break;
...
public:
    ...
    double Evaluate(const ValGetter& val_getter) const override {
        ...
        switch (type_) {
            case Type::UnaryPlus:
                res = operand_->Evaluate(val_getter);
                break;
...
double FormulaAST::Execute(const ValGetter& val_getter) const {
    return root_expr_->Evaluate(val_getter);
}
